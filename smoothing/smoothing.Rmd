...
# use R built-in supersmoother
ssmooth.val  = supsmu(x,y) 

# apply running line smoothing with different smoothing parameters
rlsmooth.vals = mapply(RLSmoother, h+1, MoreArgs = list(y,x)) 
...
**#######################################################################################################**
                                   function(x){(x^3) * cos((x+1.4)/2)}

**### The "datasmooth.txt" contains <x, y> pairs generated from function(x){(x^3) * cos((x+1.4)/2)} with random noise.**

**### Create a scatter plot of the data and overlay the true relationship line on the plot.**
...
x.plot <- seq(min(x),max(x),length.out=1000)
y.plot <- s(x.plot)
plot(x, y, xlab="Predictor", ylab="Response", main="Scatter plot and true relation")
lines(x.plot, y.plot, lty=2, lwd=2, col="red")
...
#lines(x, s3, col="blue3", lwd=1)
#lines(x, s43, lty=5, col="green4", lwd=2)
legend("bottomright",c("True relation"),
      lty=c(2), col=c("red"))
```
...
**###  Try normal kernel smoothing with different bandwidths on "datasmooth.txt". --------------------**
\vspace{6mm}
...
**### Apply cubic spline with different spars on "datasmooth.txt". -------------------------------- **
...
**### "newDatasmooth.txt" contains the <x, y> pairs generated from the same function. Utilise this new dataset to estimate mean squared error and select best bandwidths and spars for kernel smoother and cubic spline, respectively.**
...
cubicSpline.fit <- smooth.spline(x=x, y=y, cv=TRUE)
lines(cubicSpline.fit, col="green4", lwd=2)

legend("bottom",c("True relation","Kernel smoothing","Spline smoothing"),
      lty=c(2,1,1), col=c("red","purple","green4"))
...
x.plot <- seq(min(x),max(x),length.out=1000)
y.plot <- s(x.plot)
plot(x, y, xlab="Predictor", ylab="Response", main="Datasmooth.txt")
lines(x.plot, y.plot, lty=2, lwd=2, col="red")


# estimate optimal bandwidth using direct plug-in method in Ruppert, Sheather and Wand (1995).
# MAKE THE KERNAL SMOOTHING WITH PURPLE COLOR
optimBw <- dpill(x, y)
fit <- locpoly(x, y, kernel="normal", bandwidth=optimBw)
lines(fit, col="purple", lwd=2)

# cubic spline smoothing with green line
cubicSpline.fit <- smooth.spline(x=x, y=y, cv=TRUE)
lines(cubicSpline.fit, col="green4", lwd=2)
...
** BOTH OF THE SMOOTHER FITING ALMOST THE SAME WAY**
